...
(async () => {
  ...
  try {
    ...
    const biography = {...profile.emptyBiography(), health:"Synthetic health note", family:[{name:"Sample member",relationship:"Sibling",birthYear:"1990",occupation:"Engineer",notes:""}]};
    const biographyProfile = {...p,biography};
    const biographyId = await store.saveShare(biographyProfile);
    assert.deepEqual(await store.readShare(biographyId),biographyProfile);
    biography.health="Updated health note";
    assert.notEqual(await store.saveShare({...p,biography}),biographyId);
    assert.equal((await store.readShare(biographyId)).biography.health,"Synthetic health note");
