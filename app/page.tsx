...
import { DRAFT_KEY, validateProfile, decodeProfile } from "@/lib/sharing";
...
export default function Home() {
 ...
 useEffect(()=>{
  try { if(window.location.hash.startsWith("#p=")){setShared(true);setPreview(true);setProfile(decodeProfile(window.location.hash.slice(3)));}else{const raw=localStorage.getItem(DRAFT_KEY);if(raw)setProfile(validateProfile(JSON.parse(raw)));} }
  catch { if(window.location.hash.startsWith("#p=")){setShared(true);setInvalid(true);}else{setNotice("Không thể đọc bản nháp trên trình duyệt này. Bạn có thể nhập bản sao JSON.");} }
  setReady(true);
 ...
 if(invalid)return <main className="invalid-share"><Code2 size={40}/><h1>Không thể mở hồ sơ này.</h1><p>Link có thể bị thiếu hoặc không hợp lệ. Hãy nhờ người gửi sao chép lại toàn bộ link.</p></main>;
 ...
 <div className="stack-band reveal"><div className="shell"><span className="mono">MY EVERYDAY TOOLKIT</span><div>{profile.skills.split(",").slice(0,6).map((s,i)=><span key={i}><ToolIcon name={s} size={18}/>{s.trim()}</span>)}</div></div></div>
